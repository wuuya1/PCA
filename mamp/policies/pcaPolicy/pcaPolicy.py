...
class PCAPolicy(Policy):
    def __init__(self):
        Policy.__init__(self, str="PCAPolicy")
        self.type = "internal"
        self.now_goal = None
...
def compute_v_pref(agent):
    ...
    if dist_goal < Config.NEAR_GOAL_THRESHOLD:
        ...
        v_pref[1] = 0.0
    agent.v_pref = v_pref
    v_pref = np.array([v_pref[0], v_pref[1]])
    return v_pref


def intersect(v_pref, RVO_BA_all, agent):
    norm_v = agent.pref_speed
    suitable_V = []
    unsuitable_V = []

    Theta = np.arange(0, 2 * pi, step=pi / 32)
    RAD = np.linspace(0.02, norm_v, num=5)
    v_list = [v_pref[:]]
    for theta, rad in product(Theta, RAD):
        new_v = np.array([cos(theta), sin(theta)]) * rad
    ...
    # ----------------------
    if agent.pref_speed >= 0.44:    # Adjust this parameter according to agents' preferred speed.
        delta_vopt = 1e-1
    else:
        delta_vopt = 1e-2
    ...
    if suitable_V:
        ...
        if len(suitable_V) > 1:
            vA_post = select_right_side(suitable_V, agent.vel_global_frame, epsilon=delta_vopt, opt_num=6)
        else:
        ...
        if len(unsuitable_V) > 1:
            vA_post = select_right_side(unsuitable_V, agent.vel_global_frame, epsilon=delta_vopt, opt_num=6)
        else:
