import numpy as np
from mamp.sensors.Sensor import Sensor
from mamp.envs import Config
from mamp.util import compute_time_to_impact, vec2_l2_norm
import operator
...
class OtherAgentsStatesSensor(Sensor):
    ...
    def sense(self, agents, agent_index):
        ...
        for other_agent in agents:
            ...
            other_obs = np.array([other_agent.group,
                                  other_agent.pos_global_frame[0],
                                  other_agent.pos_global_frame[1],
                                  dist_2_other])

            if other_agent_count == 0:
                host_agent.other_agent_states[:] = other_obs

            other_agents_states.append(other_obs)
            other_agent_count += 1

        host_agent.num_other_agents_observed = other_agent_count
        other_agents_states_array = np.reshape(other_agents_states, (-1, 4))
        return other_agents_states_array

    def sense1(self, agents, agent_index, top_down_map=None):
        """ Go through each agent in the environment, and compute its relative position, vel, etc. and put into an array
        ...
        for other_agent in clipped_sorted_agents:
            ...
            rel_pos_to_other_global_frame_exp = other_agent.goal_global_frame - host_agent.goal_global_frame
            # project other elements onto the new reference frame
            rel_pos_to_other_global_frame = other_agent.pos_global_frame - host_agent.pos_global_frame
            dist_2_other = np.linalg.norm(rel_pos_to_other_global_frame) - host_agent.radius - other_agent.radius
            rel_pos_diff = rel_pos_to_other_global_frame - rel_pos_to_other_global_frame_exp
            ...
            other_obs = np.array([rel_pos_diff[0],
                                  rel_pos_diff[1],
            ...
            if other_agent_count == 0:
                host_agent.other_agent_obs[:] = other_obs
