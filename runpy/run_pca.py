...
def simulation1_in_circle(num_agents, rad=10.0):
    center = [0.0, 0.0]
    test_qi, test_qf = [], []
...
def set_random_pos(agent_num, r=40):
    """
    The square is 80 * 80
    The maximum number of agents is 81*81.
    """
    r = int(r)
    poses = []
    for i in range(-r, r+1):
        for j in range(-r, r+1):
            pos = np.array([i, j, random.uniform(0.0, 2*np.pi)])
            poses.append(pos)
...
def build_agents():
    """
    simulation 1: agents' num is 10-150, circle's radius is 40, random's radius is 40
    ...
    radius = 0.2
    pref_speed = 0.22        # turtleBot3's max speed is 0.22 or 0.26 m/s, and 2.84 or 1.82 rad/s
    agents = []
...
def build_obstacles():
    """
        simulation1: r = 16.0, agents' num is 100, radius of circle is 40
        simulation2: r = 16.0, agents' num is 100, radius of circle is 40
        exp_real-world: obstacles is None
    """
    obstacles = []
    r = 16.0
...
if __name__ == '__main__':
    ...
    # is_back2start is True in experiment1 and is False in experiment2
    for agent in agents:
    ...
    log_save_dir = os.path.dirname(os.path.realpath(__file__)) + '/../draw/pca/log/'
